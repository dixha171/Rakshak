"""
Regression + exploit-replay harness for Target 2 (cwe_416_use_after_free).
"""
import subprocess
import sys
import os
...
BIN_PATH = os.path.join(BIN_DIR, "auth_session_harness")
...
def build(cc="gcc", extra_flags=None):
    ...
    cmd = [
        cc, "-g", "-O0",
        "-DAUTH_SESSION_STANDALONE",
        os.path.join(ROOT, "src", "auth_session.c"),
        "-o", BIN_PATH,
    ] + flags
...
if __name__ == "__main__":
    ...
    if crashed:
        print("[PRE-PATCH] Exploit replay reproduced the use-after-free (expected before patch).")
